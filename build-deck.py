...
# ///
"""Build the portable Anki package and TSV; never touch a live Anki profile."""
...
MODEL_NAME = "Georgian Essentials — Practical + Listening"
DECK_NAME = "Georgian - Essentials"
...
TEMPLATES = [
    ...
    },
    {
        ...
        "qfmt": '{{#Audio}}<div class="hint">Listen, repeat aloud, then identify the words.</div>'
        '<div class="audio">{{Audio}}</div>{{/Audio}}',
        "afmt": '<div class="hint">Listen and repeat</div><div class="audio">{{Audio}}</div>'
...
with (ROOT / "practical.tsv").open(encoding="utf-8", newline="") as stream:
    for lineno, row in enumerate(csv.reader(stream, delimiter="\t"), 1):
        ...
        if len(row) != 4 or not all(row[:2] + row[3:]):
            raise ValueError(f"practical.tsv:{lineno}: Front, Back and Tags are required")
        front, back, notes, tags = row
        if "recognition" not in tags.split() and not re.fullmatch(r"[\u10d0-\u10ff\s.,?!…\-]+", back):
            raise ValueError(f"Unexpected non-Georgian characters in answer: {back}")
        clip_id = manifest["cards"][front]
...
for clip in clips:
    rows.append([
        "Read aloud:<br>" + html.escape(clip["text"]),
        html.escape(clip["meaning"]),
        html.escape(clip["notes"]),
...
model = genanki.Model(
    1928316457, MODEL_NAME,
    fields=[{"name": name} for name in ["Front", "Back", "Notes", "Audio"]],
    templates=TEMPLATES, css=CSS,
)
deck = genanki.Deck(1824906731, DECK_NAME)
deck.description = "Practical Georgian with usage notes, recognition prompts and synthetic pronunciation audio. No transliteration. See the included README for study and import instructions."
for index, row in enumerate(rows, 1):
    deck.add_note(genanki.Note(
        model=model, fields=row[:4], tags=row[4].split(),
        guid=genanki.guid_for("georgian-essentials-v1", original_fronts.get(row[0], row[0])), due=index,
    ))
package = genanki.Package(deck)
package.media_files = media_files
apkg = OUT / "Georgian-Essentials.apkg"
package.write_to_file(str(apkg))

with (OUT / "cards.tsv").open("w", encoding="utf-8", newline="") as stream:
    stream.write("#separator:Tab\n#html:true\n#tags column:5\n")
    stream.write(f"#notetype:{MODEL_NAME}\n#deck:{DECK_NAME}\n")
...
for row in rows:
    ...
    preview.append(f'<article><small>{html.escape(row[4])}</small><h2>{browser_audio(row[0])}</h2>'
                   f'<details><summary>Show answer</summary><h3>{browser_audio(row[1])}</h3>{player}<p>{row[2]}</p></details></article>')
(OUT / "preview.html").write_text("""<!doctype html>
<html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>Georgian - Essentials</title>
...
<h1>Georgian - Essentials</h1>
<p>Practice the answer aloud before revealing it.</p>
<label for="search">Find a word, phrase or tag</label><input id="search" type="search" placeholder="e.g. core, transport, pronunciation, წყალი">
""" + "\n".join(preview) + """
...
with zipfile.ZipFile(apkg) as archive:
    ...
    with tempfile.TemporaryDirectory(prefix="georgian-anki-check-") as temp:
        ...
        connection.close()
with (OUT / "cards.tsv").open(encoding="utf-8", newline="") as stream:
